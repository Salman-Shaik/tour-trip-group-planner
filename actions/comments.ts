...
export async function deleteComment(inviteCode:string,listingId:string,commentId:string){const database=await readDatabase();const trip=database.trips.find((item)=>item.inviteCode===inviteCode);if(!trip)return;const participant=await getCurrentParticipant(trip,database);if(!participant)return;await updateDatabase((data)=>{const listingBelongsToTrip=data.listings.some((item)=>item.id===listingId&&item.tripId===trip.id);const comment=data.comments.find((item)=>item.id===commentId&&item.listingId===listingId);if(!listingBelongsToTrip||!comment||comment.participantId!==participant.id)return;data.comments=data.comments.filter((item)=>item.id!==commentId);});revalidatePath(`/trip/${inviteCode}/listings/${listingId}`);}
