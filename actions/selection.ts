...
export async function selectFinalStay(inviteCode:string,listingId:string){
  const database=await readDatabase();const trip=database.trips.find((item)=>item.inviteCode===inviteCode);if(!trip||trip.status!=="ACTIVE"||!(await isTripCreator(trip))||isVotingOpen(trip))return;if(!database.listings.some((item)=>item.id===listingId&&item.tripId===trip.id))return;if(currentVotingRound(trip)===2&&!trip.finalistListingIds?.includes(listingId))return;
