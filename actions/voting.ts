...
export async function configureVoting(inviteCode:string,_state:unknown,formData:FormData){
  const database=await readDatabase();const trip=database.trips.find((item)=>item.inviteCode===inviteCode);if(!trip||!(await isTripCreator(trip)))return{message:"Only the host can configure voting."};if(trip.status!=="ACTIVE")return{message:"Reopen this trip before changing voting."};
...
export async function lockVoting(inviteCode:string){const database=await readDatabase();const trip=database.trips.find((item)=>item.inviteCode===inviteCode);if(!trip||trip.status!=="ACTIVE"||!(await isTripCreator(trip)))return;await updateDatabase((data)=>{const target=data.trips.find((item)=>item.id===trip.id);if(!target)return;const state=effectiveVotingState(target);target.votingState=state.startsWith("ROUND_2")?"ROUND_2_LOCKED":"ROUND_1_LOCKED";target.votingDeadline=null;target.updatedAt=new Date().toISOString();});refresh(inviteCode);}

export async function reopenVoting(inviteCode:string){const database=await readDatabase();const trip=database.trips.find((item)=>item.inviteCode===inviteCode);if(!trip||trip.status!=="ACTIVE"||!(await isTripCreator(trip)))return;await updateDatabase((data)=>{const target=data.trips.find((item)=>item.id===trip.id);if(!target)return;const state=effectiveVotingState(target);target.votingState=state.startsWith("ROUND_2")?"ROUND_2_OPEN":"ROUND_1_OPEN";target.votingDeadline=null;target.updatedAt=new Date().toISOString();});refresh(inviteCode);}

export async function startFinalRound(inviteCode:string){const database=await readDatabase();const trip=database.trips.find((item)=>item.inviteCode===inviteCode);if(!trip||trip.status!=="ACTIVE"||!(await isTripCreator(trip))||effectiveVotingState(trip)!=="ROUND_1_LOCKED")return;const listings=database.listings.filter((item)=>item.tripId===trip.id);if(!usesSecondRound(trip,listings.length))return;const ids=chooseFinalists(listings,database.votes);await updateDatabase((data)=>{const target=data.trips.find((item)=>item.id===trip.id);if(target){target.finalistListingIds=ids;target.votingState="ROUND_2_OPEN";target.votingDeadline=null;target.updatedAt=new Date().toISOString();}});refresh(inviteCode);}
