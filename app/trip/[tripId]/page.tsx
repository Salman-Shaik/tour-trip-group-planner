...
export default async function TripPage({ params }:Props) {
  ...
  const listings = database.listings.filter((item)=>item.tripId===trip.id).map((listing)=>{
    ...
    database.votes.filter((vote)=>vote.listingId===listing.id).forEach((vote)=>counts[vote.value]++);
    const currentVote=participant?database.votes.find((vote)=>vote.listingId===listing.id&&vote.participantId===participant.id)?.value??null:null;
