...
export type TripLifecycle="ACTIVE"|"PAST"|"COMPLETED";
...
export function groupTrips<T extends Trip>(trips:T[],today?:string){
  return trips.reduce((groups,trip)=>{groups[getTripLifecycle(trip,today)].push(trip);return groups;},{ACTIVE:[] as T[],PAST:[] as T[],COMPLETED:[] as T[]});
}
